...
def publisher():
    ...
    return 'Obtaining Publisher 3 Messages'
...
def advertise_button():
    ...
    message = {'pub_identifier': 3, 'topic_name': topics_id[b+1], 'topic_identifier': b+1}
    requests.post('http://mars_broker1:8002/advertise', json= {'a': message})
    application.logger.info('Advertised')
    return 'Publisher 3 has advertised a new topic - ' + topics_id[b+1]

@application.route('/deadvertise', methods = ['POST', 'GET'])
def deadvertise():
    message = {'pub_id': 3, 'topic_identifier': 3}
    requests.post('http://mars_broker:8002/deadvertise', json = {'a': message})
    application.logger.info('Deadvertise')
    return 'Publisher 3 has de-advertised a topic!'
