...
def publisher():
    ...
    for each_topic_identifier in all_topic_identifiers:
        result = {'message': output.get(topics_id[each_topic_identifier[0]]), 'pub_identifier': 2, 'topic_identifier': each_topic_identifier[0]}
        requests.post('http://mars_broker1:8002/event', json= {'a': result})
    application.logger.info('Published')
    return 'Obtaining Publisher 2 Messages'
